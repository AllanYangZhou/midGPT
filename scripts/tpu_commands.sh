...
function _tpu_copy {
    ...
    tpu_ips=$(_tpu_ips $tpu_zone $tpu_project $tpu_name)
    for host in $tpu_ips; do
        rsync -avPI --exclude=logs --exclude=__pycache__ --exclude=.git --exclude=mid_env $PROJECT_HOME/$PROJECT_NAME $host:~/ &
    done
    wait &> /dev/null
    sleep 1s
...
function _tpu_launch {
    ...
    tpu_ips=$(_tpu_ips $tpu_zone $tpu_project $tpu_name)
    for host in $tpu_ips; do
        ssh $host "tmux new -d -s python ~/midGPT/launch.py --config=openwebtext --debug" &
    done
    wait &> /dev/null
}
